...
{
    public class Vhs_Rewind
    {
        public Vhs_Rewind(string inputVideo, string outputVideo, double duration)
        {
            string VhsRewind =
                $"-i {inputVideo} " +
                $"-filter_complex " +
                ...
                $"-map \"[out]\" -c:a copy -y {outputVideo}";

            Console.WriteLine(VhsRewind);

            Utils.Utils.ExecuteFFmpegCommand(VhsRewind);
        }
    }
}
