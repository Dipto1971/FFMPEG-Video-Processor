...
namespace Testing.Video_Effects
{
    public class Soar
    {
        public Soar(string inputVideo, string outputVideo, double duration, int fps, int height, int width)
        {
            ...
            double baseRotation = Math.PI / 30;
            double rotationIncrement = Math.PI / 30;
            ...
            string command = 
                ...
                $"[7] trim=start={firstDurationEnd + baseDelay + delayIncrement * 2}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 2}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 2}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 2}):oh=roth({baseRotation + rotationIncrement * 2}) [rotated_2];" +
                $"[8] trim=start={firstDurationEnd + baseDelay + delayIncrement * 3}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 3}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 3}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 3}):oh=roth({baseRotation + rotationIncrement * 3}) [rotated_3];" +
                $"[9] trim=start={firstDurationEnd + baseDelay + delayIncrement * 4}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 4}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 4}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 4}):oh=roth({baseRotation + rotationIncrement * 4}) [rotated_4];" +
                $"[10] trim=start={firstDurationEnd + baseDelay + delayIncrement * 5}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 5}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 5}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 5}):oh=roth({baseRotation + rotationIncrement * 5}) [rotated_5];" +
                $"[11] trim=start={firstDurationEnd + baseDelay + delayIncrement * 6}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 6}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 6}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 6}):oh=roth({baseRotation + rotationIncrement * 6}) [rotated_6];" +
                $"[12] trim=start={firstDurationEnd + baseDelay + delayIncrement * 7}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 7}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 7}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 7}):oh=roth({baseRotation + rotationIncrement * 7}) [rotated_7];" +
                $"[13] trim=start={firstDurationEnd + baseDelay + delayIncrement * 8}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 8}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 8}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 8}):oh=roth({baseRotation + rotationIncrement * 8}) [rotated_8];" +
                $"[14] trim=start={firstDurationEnd + baseDelay + delayIncrement * 9}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 9}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 9}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 9}):oh=roth({baseRotation + rotationIncrement * 9}) [rotated_9];" +
                $"[15] trim=start={firstDurationEnd + baseDelay + delayIncrement * 10}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 10}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 10}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 10}):oh=roth({baseRotation + rotationIncrement * 10}) [rotated_10];" +
                $"[16] trim=start={firstDurationEnd + baseDelay + delayIncrement * 11}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 11}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 11}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 11}):oh=roth({baseRotation + rotationIncrement * 11}) [rotated_11];" +
                $"[17] trim=start={firstDurationEnd + baseDelay + delayIncrement * 12}:end={secondDurationEnd}, setpts=(PTS-STARTPTS + {baseDelay + delayIncrement * 12}/TB), fps=fps={fps}, scale=iw-10:ih-10, rotate={baseRotation + rotationIncrement * 12}:fillcolor=none:ow=rotw({baseRotation + rotationIncrement * 12}):oh=roth({baseRotation + rotationIncrement * 12}) [rotated_12];"+


                // Overlaying part is not working correctly
                $"[collage_2_base][rotated_0] overlay=x={-width / left_shift}:y={height / 50} :enable='between(t,{baseDelay},3)' [v0overlayed];" +
                $"[v0overlayed][rotated_1] overlay=x={-width / left_shift}:y={height / 50} :enable='between(t,{baseDelay + delayIncrement},3)' [v1overlayed];" +
                $"[v1overlayed][rotated_2] overlay=x={-width / left_shift}:y={height / 45} :enable='between(t,{baseDelay + delayIncrement * 2},3)' [v2overlayed];" +
                $"[v2overlayed][rotated_3] overlay=x={-width / left_shift}:y={height / 45} :enable='between(t,{baseDelay + delayIncrement * 3},3)' [v3overlayed];" +
                $"[v3overlayed][rotated_4] overlay=x={-width / left_shift}:y={height / 40} :enable='between(t,{baseDelay + delayIncrement * 4},3)' [v4overlayed];" +
                $"[v4overlayed][rotated_5] overlay=x={-width / left_shift}:y={height / 35} :enable='between(t,{baseDelay + delayIncrement * 5},3)' [v5overlayed];" +
                $"[v5overlayed][rotated_6] overlay=x={-width / left_shift}:y={height / 35} :enable='between(t,{baseDelay + delayIncrement * 6},3)' [v6overlayed];" +
                $"[v6overlayed][rotated_7] overlay=x={-width / left_shift}:y={height / 30} :enable='between(t,{baseDelay + delayIncrement * 7},3)' [v7overlayed];" +
                $"[v7overlayed][rotated_8] overlay=x={-width / left_shift}:y={height / 30} :enable='between(t,{baseDelay + delayIncrement * 8},3)' [v8overlayed];" +
                $"[v8overlayed][rotated_9] overlay=x={-width / left_shift}:y={height / 25} :enable='between(t,{baseDelay + delayIncrement * 9},3)' [v9overlayed];" +
                $"[v9overlayed][rotated_10] overlay=x={-width / left_shift}:y={height / 25} :enable='between(t,{baseDelay + delayIncrement * 10},3)' [v10overlayed];" +
                $"[v10overlayed][rotated_11] overlay=x={-width / left_shift}:y={height / 20} :enable='between(t,{baseDelay + delayIncrement * 11},3)' [v11overlayed];" +
                $"[v11overlayed][rotated_12] overlay=x={-width / left_shift}:y={height / 20} :enable='between(t,{baseDelay + delayIncrement * 12},3)' [rot_overlayed];" +
